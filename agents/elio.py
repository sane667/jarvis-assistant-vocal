...
def _run_command(workspace: Path, command: str) -> str:
    ...
    try:
        result = subprocess.run(raw, cwd=workspace, shell=True, capture_output=True,
...
def _publish(task: AgentTask, message: str = ""):
    try:
        import project_hud
...
def run(task: AgentTask):
    ...
    for step in range(MAX_STEPS):
        if task.cancel_event.is_set():
            task.status = "cancelled"; task.message = "Elio a été annulé."; _publish(task, task.message); return task.message
        task.progress = f"Étape {step + 1}/{MAX_STEPS} · analyse et exécution"
        _publish(task, f"Elio · {task.progress}")
        response = llm().repondre(system, history, TOOLS)
        tool_calls = [b for b in response.content if b.type == "tool_use"]
        texts = [b.text for b in response.content if b.type == "text" and b.text]
        if not tool_calls:
            result = " ".join(texts).strip() or "Elio a terminé sans message final."
            task.progress = "Terminé"
            _publish(task, result)
            return result
        history.append({"role": "assistant", "content": response.content})
        results = []
        for call in tool_calls:
            task.progress = f"Étape {step + 1}/{MAX_STEPS} · {call.name}"
            _publish(task, f"Elio utilise {call.name}")
            try:
    ...
    task.progress = "Limite d'étapes atteinte"
    _publish(task, task.progress)
