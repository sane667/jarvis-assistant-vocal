...
class ProjectStore:
    ...
    def __init__(self, root: str | Path):
        ...
        self._active: str | None = None; self._load_active()
    ...
    @property
    def active_slug(self) -> str | None: return self._active
    def _active_file(self) -> Path: return self.root / ".tony-active.json"

    def _load_active(self):
        try:
            data = json.loads(self._active_file().read_text(encoding="utf-8")); slug = data.get("slug")
            if slug and (self.root / _slug(slug)).is_dir(): self._active = _slug(slug)
        except (OSError, ValueError, TypeError): self._active = None

    def _save_active(self):
        if self._active:
            self._active_file().write_text(json.dumps({"slug": self._active}, ensure_ascii=False, indent=2), encoding="utf-8")
        else:
    ...
    def _metadata_path(self, directory: Path) -> Path: return directory / ".tony" / "project.json"

    def _publish(self, project: Project | None, message: str = ""):
        try:
            import project_hud
            project_hud.demarrer(ouvrir=bool(reglage("projets.dashboard_auto_ouvrir", True)))
    ...
    def create(self, name: str, description: str = "") -> Project:
        ...
        project = Project(name=name, slug=slug, path=str(directory), description=description.strip(), created_at=now, updated_at=now)
        project.nodes.append({"id": slug, "label": name, "type": "project"}); self._write(project)
        self._publish(project, f'Projet "{name}" créé.'); return project
    ...
    def activate(self, slug: str) -> Project:
        ...
        if self._active and self._active != project.slug:
            try:
                previous = self.open(self._active); previous.active = False; self._write(previous)
            except FileNotFoundError: pass
        self._active = project.slug; project.active = True; project.updated_at = datetime.now(timezone.utc).isoformat()
        self._write(project); self._save_active(); self._publish(project, f'Projet "{project.name}" activé.'); return project
    ...
    def deactivate(self):
        ...
        try:
            ...
            try:
                import project_hud; project_hud.projet(None)
            except Exception: pass
    ...
    def list(self) -> list[Project]:
        ...
        for directory in sorted(self.root.iterdir()):
            if directory.is_dir() and (directory / ".tony" / "project.json").exists():
                try: projects.append(self.open(directory.name))
                except (OSError, ValueError, TypeError): pass
        return projects

    def add_node(self, project: Project, node: dict[str, Any]) -> Project:
        project.nodes.append(node); project.updated_at = datetime.now(timezone.utc).isoformat(); self._write(project)
        if project.active: self._publish(project)
        return project

    def add_edge(self, project: Project, edge: dict[str, Any]) -> Project:
        project.edges.append(edge); project.updated_at = datetime.now(timezone.utc).isoformat(); self._write(project)
        if project.active: self._publish(project)
        return project

    def _write(self, project: Project):
        directory = Path(project.path).resolve(); directory.relative_to(self.root)
        metadata = self._metadata_path(directory); metadata.parent.mkdir(parents=True, exist_ok=True)
...
def creer_projet(nom: str, description: str = "") -> str:
    project = store().create(nom, description); return f'Projet "{project.name}" créé dans {project.path}.'

def ouvrir_projet(nom: str) -> str:
    project = store().activate(nom); return f'Projet "{project.name}" ouvert. Workspace : {project.path}'

def fermer_projet() -> str:
    project = store().active
    if project is None: return "Aucun projet n'est actuellement ouvert."
    name = project.name; store().deactivate(); return f'Projet "{name}" fermé.'

def lister_projets() -> str:
    projects = store().list()
    if not projects: return "Aucun projet n'est créé."
    active = store().active_slug; return "Projets : " + "; ".join(f'- {p.name}{" (actif)" if p.slug == active else ""}' for p in projects)
